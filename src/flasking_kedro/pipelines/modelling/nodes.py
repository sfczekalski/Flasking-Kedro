...
def fit_logistic_regression(
    ...
    Returns:
        LogisticRegression: Fitted logistic regression.
    """
...
def logistic_regression_predict(model: LogisticRegression, X: np.ndarray) -> np.ndarray:
    """Predict labels.

    Args:
        model (LogisticRegression): Fitted logistic regression.
...
def evaluate_accuracy(
    ...
    Args:
        y_true (np.ndarray): Grand-truth labels.
        y_pred (np.ndarray): Predicted labels.
        dataset_name (str): Name of the dataset (train / validation / test).
    """
    acc = accuracy_score(y_true, y_pred)

    logger.info(f"{dataset_name} set accuracy: {acc}")
